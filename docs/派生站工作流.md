...
设计参考和已采纳的外部原则统一记录在 `docs/design/设计参考.md`。它是参考证据库，不是品牌规范；当前项目的强制设计规则仍以 `docs/design/设计真相.md` 为准。
...
视觉参考只提取可迁移的原则和细节，不直接复制 Apple、Vercel、Cursor、OpenAI、Linear 或 IBM 的品牌外观。参考无法映射买家任务、现有页面/Token/组件，或会引入无业务收益的复杂度时，不采用。
